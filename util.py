...
class HGNNGraph(object):
    def __init__(self, g, label, node_tags=None, node_features=None):
        '''
            g: a networkx bipartite graph as hypergraph
        ...
        self.node_ids = [v for v in g.nodes() if type(v) == int]
        self.hyperedge_ids = [v for v in g.nodes() if type(v) == str]
        _, self.hydegs = list(dict(nx.bipartite.degrees(g, self.node_ids)).values())
